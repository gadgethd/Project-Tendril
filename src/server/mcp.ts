...
async function sessionPage(session: TendrilSession, requestedPageId?: string) {
  ...
  const pageIndex = requestedPageId
    ? summaries.findIndex((page) => page.id === requestedPageId)
    : summaries.findIndex((page) => page.selected);
...
async function configureInterception(session: TendrilSession, rules: InterceptionRule[]): Promise<void> {
  ...
  for (const rule of [...rules].reverse()) {
    const handler: RouteHandler = async (route) => {
      ...
      await route.continue(rule.modifyHeaders
        ? { headers: { ...route.request().headers(), ...rule.modifyHeaders } }
        : undefined);
    };
...
export function createMcpServer(services: { manager: BrowserManager; search: SearchService; crawl: CrawlService }): McpServer {
  ...
  server.registerTool('browser_session', {
    ...
    description: 'Create, reconnect, list, inspect, monitor, export, import, reset, or close isolated Chromium sessions. Sessions are ephemeral unless a named profile is supplied.',
    inputSchema: {
  ...
  server.registerTool('browser_act', {
    ...
    return result(await session.act({
      action: input.action,
      ref: input.ref,
  ...
  server.registerTool('browser_research', {
    ...
    description: 'Start or refine a research job that searches, deduplicates sources, visits pages, and returns source-attributed evidence without an embedded LLM.',
    inputSchema: {
  ...
  server.registerTool('browser_crawl', {
    ...
    inputSchema: {
      action: z.enum(['start', 'followup', 'status', 'results', 'cancel']), jobId: z.string().optional(), url: z.string().optional(),
      followUpQueries: z.array(z.string().min(1)).min(1).max(10).optional(),
    ...
    if (input.action === 'followup') {
      if (!input.followUpQueries) throw new Error('followUpQueries is required');
      const jobs = input.followUpQueries.map((url) => crawl.followUp(input.jobId!, {
        url,
  ...
  server.registerTool('browser_capture', {
    ...
    description: 'Capture a viewport, full page, element screenshot, or PDF. Return images as MCP content, save them locally, or use saveOnly to omit inline base64 data.',
    inputSchema: {
    ...
    if (input.saveOnly) return result({
  ...
  server.registerTool('browser_storage', {
    ...
    inputSchema: { sessionId, action: z.enum(['get', 'set_cookies', 'export_cookies', 'import_cookies', 'clear']), cookies: z.array(cookie).optional(), origin: z.string().optional() },
    annotations: { readOnlyHint: false, destructiveHint: true, openWorldHint: false },
  }, wrap(async (input) => {
    const session = manager.get(input.sessionId);
  ...
  server.registerTool('browser_configure', {
    ...
    inputSchema: {
      sessionId, action: z.enum(['configure', 'intercept']).default('configure'),
      viewport: viewport.optional(), headers: z.record(z.string(), z.string()).optional(),
  ...
  }, async (uri) => ({ contents: [{ uri: uri.href, mimeType: 'application/json', text: JSON.stringify({ version: VERSION, sessions: await manager.list() }, null, 2) }] }));
