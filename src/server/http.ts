...
export async function startHttpServer(services: { manager: BrowserManager; search: SearchService; crawl: CrawlService; logger: Logger }): Promise<TendrilHttpServer> {
  ...
  app.post('/v1/sessions/:id/extract', async (request, response, next) => {
    try {
      response.json(await manager.get(request.params.id!).extractWithSafety(request.body));
      ...
      try {
        ...
        if (format === 'screenshot' || format === 'pdf') {
          ...
          if (formats.includes('content')) {
            const extracted = await session.extractWithSafety({ format: 'html' });
            output.content = extracted.data;
            warnings.push(...extracted.warnings);
          }
          if (formats.includes('markdown')) {
            const extracted = await session.extractWithSafety({ format: 'markdown' });
            output.markdown = extracted.data;
            warnings.push(...extracted.warnings);
          }
          if (formats.includes('accessibilityTree')) {
            const accessibilityTree = await session.snapshot({ mode: 'full' });
            output.accessibilityTree = accessibilityTree;
            warnings.push(...accessibilityTree.warnings);
          }
          if (formats.includes('screenshot')) output.screenshot = await session.capture({ format: 'png', fullPage: request.body.fullPage });
          response.json({ ...output, untrustedContent: true, warnings: [...new Set([...warnings, ...mergeInjectionWarnings(output)])] });
        } else {
          if (format === 'accessibility-tree') {
            const mapped = await session.snapshot({ mode: 'full' });
            response.json({ url: request.body.url, accessibilitytree: mapped, untrustedContent: true, warnings: mapped.warnings });
          } else {
            const extracted = await session.extractWithSafety({ format: format === 'content' ? 'html' : format });
            response.json({ url: request.body.url, [format.replace('-', '')]: extracted.data, untrustedContent: true, warnings: extracted.warnings });
          }
        }
      } catch (error) { next(error); }
