...
$('challenge').onclick=async()=>{if(!state.session)return log('Create a session first');try{const data=await api('/v1/sessions/'+state.session+'/challenge');log(data);if(data.detected&&!data.headed)log({...data,next:'Restart Tendril with --headed, then solve manually in the preserved session.'})}catch(e){log(e.message)}};
