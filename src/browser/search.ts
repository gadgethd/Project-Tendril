  ...
  private async fetchEvidence(results: SearchResult[], query: string): Promise<EvidenceChunk[]> {
    ...
    for (const result of results) {
      ...
      try {
        ...
        const extracted = safeExtraction.data as { title: string; markdown: string };
        const chunks = extracted.markdown.split(/\n{2,}/).map((text) => text.trim()).filter((text) => text.length >= 80);
        for (const text of chunks.slice(0, 5)) {
          evidence.push({
            sourceUrl: result.url, title: extracted.title || result.title, text: text.slice(0, 2500), query,
            ...(safeExtraction.warnings.length ? { warnings: safeExtraction.warnings } : {}),
          });
        }
      } catch (error) {
