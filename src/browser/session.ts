...
export class TendrilSession {
  ...
  async navigate(options: {
    ...
    const timeout = boundedTimeout(this.config.navigationTimeoutMs, options.deadlineMs);
    const onAbort = (): void => { void page.close({ runBeforeUnload: false }).catch(() => undefined); };
    ...
    try {
      if (action === 'goto') {
        ...
        throwIfAborted(options.signal);
        const gotoOptions = { waitUntil: options.waitUntil ?? 'domcontentloaded', timeout } as const;
        try {
      ...
      } else if (action === 'back') response = await page.goBack({ waitUntil: options.waitUntil ?? 'domcontentloaded', timeout });
      else if (action === 'forward') response = await page.goForward({ waitUntil: options.waitUntil ?? 'domcontentloaded', timeout });
      else response = await page.reload({ waitUntil: options.waitUntil ?? 'domcontentloaded', timeout });
      throwIfAborted(options.signal);
    } finally {
