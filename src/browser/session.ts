...
import { assertPathWithinOwnedRoot, assertPathWithinRoots, type Logger, newId, pathWithinOwnedRoot, withTimeout } from '../util.js';
...
export class TendrilSession {
  ...
  async fetchText(url: string, pageId?: string, signal?: AbortSignal): Promise<{ status: number | null; text: string }> {
    ...
    for (let redirects = 0; redirects <= 5; redirects += 1) {
      ...
      const result = await new Promise<{ status: number; text: string; location?: string }>((resolve, reject) => {
        ...
        const onAbort = (): void => {
          request.destroy(Object.assign(new Error('Fetch was cancelled'), { name: 'AbortError' }));
        };
  ...
  async snapshot(
    ...
    const previousContent = options.previousSnapshotId ? this.snapshotContents.get(options.previousSnapshotId) : [...this.snapshotContents.values()].at(-1);
    const maxChars = Math.min(options.maxChars ?? this.config.maxSnapshotChars, 100_000);
  ...
  private continueSnapshot(cursor: string, requestedMax?: number): SnapshotResult {
    ...
    try {
      ...
      throw new TendrilError('INVALID_URL', 'Invalid snapshot cursor');
    }
    ...
    const result: SnapshotResult = {
      ...
      title: '',
      mode: 'full',
      content: content.slice(parsed.offset, nextOffset),
  ...
  private resolveTarget(ref: string): { page: Page; frame: Frame; target: ElementTarget } {
    ...
    if (!page || page.url() !== target.pageUrl)
      throw new TendrilError('STALE_ELEMENT_REF', `Element reference ${ref} is stale after navigation; take a new snapshot`);
  ...
  async act(options: {
    ...
    switch (options.action) {
      ...
      case 'uncheck':
        ...
        break;
      case 'press':
        await locator.press(options.key ?? 'Enter');
        break;
      case 'scroll':
        await locator.evaluate((element, delta) => element.scrollBy(delta.x, delta.y), { x: options.deltaX ?? 0, y: options.deltaY ?? 600 });
        break;
      case 'drag': {
  ...
  async extract(options: {
    ...
    let result: unknown;
    if (options.selector) {
      result = await page.locator(options.selector).evaluateAll((nodes) =>
        nodes.map((node) => ({
  ...
  async capture(options: BrowserCaptureOptions): Promise<BrowserCaptureResult> {
    ...
    if (type === 'pdf') {
      ...
      buffer = target
        ? await target.frame
            .locator(target.target.selector)
            .first()
            .screenshot({ type, quality: type === 'jpeg' ? (options.quality ?? 80) : undefined })
