...
import { fetchTextWithPolicy } from '../security/fetch-text.js';
...
import {
  boundedSnapshotFrameUrls,
  boundedSnapshotTitle,
  boundedSnapshotUrl,
...
export class TendrilSession {
  ...
  static async create(
    ...
    dependencies: SessionCreateDependencies = {},
  ...
  private async invalidatePageRefs(pageId: PageId): Promise<void> {
    ...
    await Promise.all(
      [...affectedSnapshots].flatMap((snapshot) => {
  ...
  private async storeSnapshot(options: {
    ...
    const normalizedCanonical = options.canonicalContent === undefined ? undefined : normalizeStoredSnapshotLines(options.canonicalContent);
    ...
    const canonicalBoundary = (normalizedCanonical?.content.length ?? 0) > MAX_STORED_SNAPSHOT_CHARS ? (cappedCanonical?.lastIndexOf('\n') ?? -1) : -1;
    ...
    await Promise.all(
      (options.documents ?? []).filter((document) => !record.documents.has(document)).map((document) => document.dispose().catch(() => undefined)),
    );
    const storedCharacters = (): number =>
      [...this.snapshots.values()].reduce((total, snapshot) => total + snapshot.fullContent.length + (snapshot.canonicalContent?.length ?? 0), 0);
  ...
  private snapshotChunk(record: StoredSnapshot, offset: number, maxChars: number, nodes?: SnapshotResult['nodes']): SnapshotResult {
    ...
    if (!truncated && offset === 0 && nodes && record.fullContent.length + JSON.stringify(nodes).length <= maxChars) result.nodes = nodes;
  ...
  async listPages(): Promise<PageSummary[]> {
    return Promise.all(
      [...this.pagesById.entries()].map(async ([id, page]) => ({
        id,
        url: page.url(),
  ...
  async navigate(options: {
    ...
    signal?: AbortSignal;
    deadlineMs?: number;
  }): Promise<{ url: string; title: string; status: number | null; mimeType?: string }> {
    ...
    if (action === 'goto') {
      ...
      try {
        ...
        throw new TendrilError('INVALID_URL', `Invalid URL: ${options.url}`, { cause: error });
      }
  ...
  async fetchText(
    ...
    return fetchTextWithPolicy(this.networkPolicy, url, {
      ...options,
      timeoutMs: this.config.navigationTimeoutMs,
      maxBytes: Math.min(requestedMaxBytes, this.config.maxResponseBodyBytes),
  ...
  async snapshot(
    ...
    if (mode === 'reader') {
      ...
      if (
        url !== boundedSnapshotUrl(url) ||
        extracted.title !== boundedSnapshotTitle(extracted.title) ||
      ...
      const base: SnapshotResult = {
        ...
        pageId,
        url: boundedSnapshotUrl(url),
        title: boundedSnapshotTitle(extracted.title),
        frameUrls,
        mode,
        content: '',
        truncated: false,
    ...
    const created = await createSnapshot({
      ...
      pageId,
      mode,
      maxChars: MAX_STORED_SNAPSHOT_CHARS,
  ...
  async act(options: {
    ...
    files?: string[];
    submit?: boolean;
  }): Promise<{ url: string; snapshot?: SnapshotResult }> {
    this.touch();
  ...
  async extract(options: {
    pageId?: string;
    format?: 'all' | 'html' | 'markdown' | 'text' | 'links' | 'metadata' | 'forms' | 'tables';
    selector?: string;
  }): Promise<unknown> {
    const page = this.currentPage(options.pageId);
    let result: unknown;
    if (options.selector) {
      const responseLimit =
        Number.isFinite(this.config.maxResponseBodyBytes) && this.config.maxResponseBodyBytes > 0 ? Math.floor(this.config.maxResponseBodyBytes) : 1;
        ...
        ({ selector, limits }) => {
          ...
          const cloneBounded = (root: Element, maxChars: number): { html: string; truncated: boolean } => {
            ...
            const copy = (source: Element, depth: number): Element | undefined => {
              if (depth > limits.maxDepth || cloneNodesRemaining <= 0) {
                truncated = true;
                return undefined;
              }
              ...
              if (serializationRemaining < elementCost) {
                truncated = true;
                return undefined;
              }
              ...
              for (let index = 0; index < attributes; index += 1) {
                ...
                if (serializationRemaining <= baseCost) {
                  truncated = true;
                  break;
                }
                ...
                const value =
                  sensitiveControl.test(lowerName) || (isSensitiveControl(source) && lowerName === 'value')
                    ? '[redacted]'
              ...
              for (let child = source.firstChild; child; child = child.nextSibling) {
                if (serializationRemaining <= 0 || cloneNodesRemaining <= 0) {
                  truncated = true;
                  break;
                }
                if (child.nodeType === 3) {
          ...
          for (let index = 0; index < count && payloadRemaining > 32; index += 1) {
            ...
            for (let attributeIndex = 0; attributeIndex < attributeCount && payloadRemaining > 8; attributeIndex += 1) {
              ...
              const rawValue =
                sensitiveControl.test(lowerName) || (isSensitiveControl(node) && lowerName === 'value')
                  ? '[redacted]'
                  : ['href', 'src', 'action', 'formaction', 'poster'].includes(lowerName)
  ...
  async extractWithSafety(options: {
    pageId?: string;
    format?: 'all' | 'html' | 'markdown' | 'text' | 'links' | 'metadata' | 'forms' | 'tables';
    selector?: string;
    ...
    const inherited =
      typeof data === 'object' && data !== null && 'warnings' in data && Array.isArray(data.warnings)
  ...
  async responseBody(requestId: string): Promise<{ body: string; base64: boolean; contentType?: string }> {
    ...
    const result: { body: string; base64: boolean; contentType?: string } = {
  ...
  async configure(options: {
    ...
    timezoneId?: string;
    userAgent?: string;
    httpCredentials?: { username: string; password: string; origin?: string } | null;
  }): Promise<void> {
    const page = this.currentPage();
    const pageId = this.pageId(page);
    if (this.browserProcess.backend === 'obscura') {
  ...
  async detectChallenge(pageId?: string): Promise<ChallengeInfo> {
          ...
          return {
            ...
            cloudflare: Boolean(
              document.querySelector(
                '#challenge-running, #challenge-stage, .cf-browser-verification, iframe[src*="challenges.cloudflare.com"], script[src*="/cdn-cgi/challenge-platform/"]',
              ),
            ),
    ...
    if (signals.turnstile) {
      provider = 'turnstile';
      kind = 'widget';
    } else if (signals.cloudflare || /just a moment|checking your browser/.test(lowerTitle) || /cf-chl-|cloudflare ray id/.test(lowerText)) {
      provider = 'cloudflare';
      kind = 'interstitial';
    } else if (signals.recaptcha) {
      provider = 'recaptcha';
      kind = 'captcha';
    ...
    const info: ChallengeInfo = {
      ...
      url: page.url(),
      title,
      requiresHuman: provider !== undefined,
  ...
  async focusForHandoff(pageId?: string): Promise<ChallengeInfo> {
    ...
    if (this.headless && challenge.detected) {
      challenge.message =
        'This session is headless and cannot be shown. Start Tendril with --headed or create a session with headless=false, preferably using a named profile.';
    }
    return challenge;
  }
  ...
  async waitForChallenge(options: { pageId?: string; timeoutMs?: number } = {}): Promise<ChallengeInfo> {
    ...
    if (challenge.detected)
      throw new TendrilError('TIMEOUT', `Challenge was not resolved within ${timeoutMs}ms`, {
        details: challenge as unknown as Record<string, unknown>,
        retryable: true,
      });
    return challenge;
  }
